...
Tile_Chunk* get_tile_chunk(Tilemap* tilemap, u32 x, u32 y) {
    ...
    if (x < tilemap->num_world_cols && y < tilemap->num_world_rows) {
        result = &tilemap->tile_chunks[y * tilemap->num_world_cols + x];        
    }
    return result;
}
...
void set_tile_value(Memory_Arena* memory_arena,  Tilemap* tilemap, u32 x, u32 y, u32 value) {
...
u32 get_tile_value(Tilemap* tilemap, u32 x, u32 y) {
...
bool is_point_empty(Tilemap* tilemap, Tilemap_Position* world_pos) {
    bool result = false;
    u32 value = get_tile_value(tilemap, world_pos->x, world_pos->y);
    if (value == 1) { // @Temproray
        result = true;
    }
    return result;
}
